...
def assert_put_response(item_name, config, hub_address, id):
    if 'url' in config and mp.MockPowerView.mock_put_called():
        ...
        if put_response:
            ...
            if data:
                ...
                assert data == compare_data, f'Invalid Put response body={data} for "{item_name} on url={put_response.url}"'
...
def testbody_activate_scene(hub_address, config):
    ...
    for scene in scenes:
        ...
        assert_put_response('activate scene', config, hub_address, scene_id)
...
def testbody_activate_scene_collection(hub_address, config):
    ...
    # Only test one collection since it is only supported on V3, and uses a Get request, not Put, so it will be run by the test when using the hub.
    if scenes:
        scene_id = scenes[0][0]
        action = PluginAction({'deviceId': hub.id, 'props': {'sceneCollectionId': scene_id}})
        plg.activateSceneCollection(action)

        assert_put_response('activate scene collections', config, hub_address=hub_address, id=scene_id)
...
def testbody_set_shade_position(hub, config):
